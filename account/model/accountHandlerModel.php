...
function updateFamily($data, $type, $userId)
{
    ...
    if ($type == 3) // Sibling
    {
        foreach ($data as $fam) 
        {
            ...
            if ($id != "") $exists = check_exist_multiple(['table' => 'user_family', 'column' => ['user_id' => ['=', $userId], 'id' => ['=', $id], 'ay_id' => ['=', $acadYear], 'sem_id' => ['=', $defaultYear]]]);
...
function set_applicant_status($data)
{
    ...
    if ($decision == 4)
    {
        ...
        $updateStatus = update_applicant_status($id, 2); // Setting the Account Status Back to 0
...
function deleteUserRequest($data)
{
    ...
    $notifData = [
        'user_id'       => get_user_id_notification($typeArr, $except),
        'notif_type'    => 11,
